...
class Admin::Manager < ActiveRecord::Base
  ...
  validates :manager_name, :presence => true, uniqueness: true , :length => { :maximum => 50 } 
  ...
  def self.authenticate(username_or_email="", login_password="")    
    ...
    manager = Admin::Manager.where(["lower(manager_name) = :value OR lower(manager_email) = :value", { :value => username_or_email.downcase }]).first
